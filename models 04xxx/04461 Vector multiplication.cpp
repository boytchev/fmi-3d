...
/// Project:	FMI 3D (2013)
/// File:		04461 Vector multiplication.cpp
/// Topic:		04 Vectors
/// Slide:		46
///	About:		Vector multiplication of vectors
///
/// History:
...
void drawScene()
{
	...
	glBegin( GL_LINES );
	{
		...
		glColor3f(1,0,0);
		glVertex3f( dw.x,dw.y,dw.z );
		glVertex3f( dw.x,dw.y,0 );
		glVertex3f( dw.x,dw.y,0 );
		glVertex3f( 0,0,0 );
	}
	glEnd();
...
void randomize()
{
	...
	du = du.mul(5/du.distance());
	dv = dv.mul(5/dv.distance());
...
{
	/// Open window
	if( !openWindow3D( "Vector multiplication" ) ) return 1;
	noGround();
	...
	cs = new CoordSys3D( -10,10,-10,10,-7,7,false,0,0.7,L"XYZ" );
	cs->setCenter( {0,0,0} );
	...
	w = new Vector3D( {0,0,1},0.7 );
	w->setColor( {1,0,0} );
	w->setCenter( cs->getCenter() );
