...
/// Project:	FMI 3D (2013)
/// File:		07552 Split tetradic colours.cpp
/// Topic:		07 Colours
/// Slide:		55
///	About:		A quadriple of split colours
///
/// History:
...
{
	/// Open window
	if( !openWindow2D( "Split tetradic colours" ) ) return 1;
	changeBrightness();
	changeGround();
	changeGround();
	...
	while( isRunning() )
	{
		double angle1 = atan2( obj[1]->getCenter().y,obj[1]->getCenter().x )*180/M_PI;
		...
		{
			angle1 = atan2( obj[3]->getCenter().y,obj[3]->getCenter().x )*180/M_PI-180;
		}

		processObj( 1,angle1 );
		processObj( 2,angle1+delta );
		processObj( 3,angle1+180 );
		processObj( 4,angle1-delta );
	}
