...
{
	const double k = 0.85*(mode?1-2*fabs(l-0.5):1);
	double c = pow(1-fabs(2*l-1),0.75)*s; // chroma
	double m = l-c/2;
	...
	glVertex3f( k*s*cos( h*M_PI/180.0 ),k*s*sin( h*M_PI/180.0 ),1.5*l );
}
...
ColorCyl3D::ColorCyl3D( int fromH, int toH ) : Object3D()
{
	...
	for( int j=0; j<N; j++ )
	{
		...
		{
			// outer wall
			HSLtoRGB( i,1,double( j )/N );
			HSLtoRGB( i+5,1,double( j )/N );
			HSLtoRGB( i+5,1,double( j+1 )/N );
			HSLtoRGB( i,1,double( j+1 )/N );
			// top
			HSLtoRGB( i,double( j )/N,1 );
			HSLtoRGB( i+5,double( j )/N,1 );
			HSLtoRGB( i+5,double( j+1 )/N,1 );
			HSLtoRGB( i,double( j+1 )/N,1 );
			// bottom
			HSLtoRGB( i,double( j )/N,0 );
			HSLtoRGB( i+5,double( j )/N,0 );
			HSLtoRGB( i+5,double( j+1 )/N,0 );
			HSLtoRGB( i,double( j+1 )/N,0 );
		}
		for( int i=0; i<N; i++ )
		{
			// start wall
			HSLtoRGB( fromH,double( i )/N,double( j )/N );
			HSLtoRGB( fromH,double( i+1 )/N,double( j )/N );
			HSLtoRGB( fromH,double( i+1 )/N,double( j+1 )/N );
			HSLtoRGB( fromH,double( i )/N,double( j+1 )/N );
			// end wall
			HSLtoRGB( toH,double( i )/N,double( j )/N );
			HSLtoRGB( toH,double( i+1 )/N,double( j )/N );
			HSLtoRGB( toH,double( i+1 )/N,double( j+1 )/N );
			HSLtoRGB( toH,double( i )/N,double( j+1 )/N );
		}
	}
	glEnd();
	glEndList();
...
void drawScene()
{
	...
	for(float i=10*M_PI/180; i<355*M_PI/180; i=i+3*M_PI/180)
		glVertex3f((5.5+0.9*h)*cos(-i),(5.5+0.9*h)*sin(-i),4);
	glEnd();
}
...
int main()
{
	...
	customFogColor[0]=0.7;
	customFogColor[1]=0.8;
	customFogColor[2]=0.9;

	/// Open window
	if( !openWindow3D( "HSL cylinder" ) ) return 1;
	changeGround();
	changeGround();
	changeGround();
	...
	tl = new Text3D(L"L",systemFont3D);
	tl->setColor({0,0,1});
	...
	while( isRunning() )
	{
		...
		for(int i=0; i<6; i++)
		{
			...
			q[i][mode]->setCenter( {r*h*cos(a)+dx,r*h*sin(a)+dy,0.1} );
		}
		...
		vh->setCenter({(5.5+3.6*h)*cos(-10*M_PI/180),(5.5+3.6*h)*sin(-10*M_PI/180),4});
		th->setCenter({4.7+3*h,-3.1-2*h,4.2});
