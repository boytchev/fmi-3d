...
/// Project:	FMI 3D (2013)
/// File:		07551 Split triadic colours.cpp
/// Topic:		07 Colours
/// Slide:		55
///	About:		A triplet of split colours
///
/// History:
...
{
	/// Open window
	if( !openWindow2D( "Split triadic colours" ) ) return 1;
	changeBrightness();
	changeGround();
	changeGround();
	...
	while( isRunning() )
	{
		...
		{
			double angle2 = atan2( obj[3]->getCenter().y,obj[3]->getCenter().x )*180/M_PI;
			delta = angle1-angle2;
			delta = delta+36000;
			delta = delta-360*int( delta/360 );
			if( delta<100 ) delta=100;
