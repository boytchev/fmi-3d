...
/// Project:	FMI 3D (2013)
/// File:		07542 Tetradic colours.cpp
/// Topic:		07 Colours
/// Slide:		54
///	About:		Selection of four reqularily-distributed colours
///
/// History:
...
{
	/// Open window
	if( !openWindow2D( "Tetradic colours" ) ) return 1;
	changeBrightness();
	changeGround();
	changeGround();
	...
	while( isRunning() )
	{
		...
		{
			double angle = atan2( obj[1]->getCenter().y,obj[1]->getCenter().x )*180/M_PI;
			processObj(1,angle);
			processObj(2,angle+90);
			processObj(3,angle+180);
			processObj(4,angle+270);
		} else
		if( obj[2]==activeObject )
		{
			double angle = atan2( obj[2]->getCenter().y,obj[2]->getCenter().x )*180/M_PI;
			processObj(2,angle);
			processObj(3,angle+90);
			processObj(4,angle+180);
			processObj(1,angle+270);
		} else
		if( obj[3]==activeObject )
		{
			double angle = atan2( obj[3]->getCenter().y,obj[3]->getCenter().x )*180/M_PI;
			processObj(3,angle);
			processObj(4,angle+90);
			processObj(1,angle+180);
			processObj(2,angle+270);
		} else
		{
			double angle = atan2( obj[4]->getCenter().y,obj[4]->getCenter().x )*180/M_PI;
			processObj(4,angle);
			processObj(1,angle+90);
			processObj(2,angle+180);
			processObj(3,angle+270);
		}
	}
