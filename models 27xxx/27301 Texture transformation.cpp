...
/// Project:	FMI 3D (2013)
/// File:		27301 Texture transformation.cpp
/// Topic:		27 Textures
/// Slide:		30
///	About:		Matrix transformtion of texture mapping
///
/// History:
...
void drawScene()
{
	...
	glPushMatrix();
	glRotated(13*t,0,0,1);
	glTranslated(2*sin(0.3*t),2*cos(0.27*t),0);
...
{
	/// Open window
	if( !openWindow3D( "Texture transformation" ) ) return 1;
	changeGround();
	changeGround();
	changeGround();
	...
	GLuint tex = loadTexture("27271 Texture Mona Lisa.jpg");
	...
	b->setTexture( tex );
	b->setCenter({0,0,-10});
	b->setTextureScale({6,6});

	s = new Sphere3D(4.5);
	s->setCenter({0,0,-10});
	s->setTexture(tex);
	s->setTextureScale({14,-7});

	c = new Cylinder3D(3.5,7,false);
	c->setCenter({0,0,-10});
	c->setTexture(tex);
	c->setTextureScale({0,0,12,-3});

	e = new Cube3D(7);
	e->setCenter({0,0,-10});
	e->setTexture(tex);
	e->setTextureScale({3,-3});
