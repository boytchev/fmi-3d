...
void randomize()
{
	...
	for( int j=0; j<M; j++ )
	{
		...
		{
			y=y+random( -1,1 );
			points[j][i]->setCenter( {i+random(-1,1)-N/2,y,random(1,3)+dz} );
			points[j][i]->setColor(col[j]);
			points[j][i+1]->setCenter( {i+random(-1,1)-N/2,y+random(-0.5,0.5),random(4,6)+dz} );
			points[j][i+1]->setColor(col[j]);
		}
	}
}
int main()
{
	/// Open window
	if( !openWindow3D( "OpenGL quad strips" ) ) return 1;
