...
/// Project:	FMI 3D (2013)
/// File:		10311 CSG difference.cpp
/// Topic:		10 Composite objects
/// Slide:		31
///	About:		Constructive solid geometry - difference of spheres
///
/// History:
...
void drawScene()
{
	...
	GLdouble cp1a[4] = {0,1,0,-cut};
	GLdouble cp2a[4] = {0,1,0,-cut};
	glClipPlane(GL_CLIP_PLANE2,cp1a);
	s1->drawImage();
	glClipPlane(GL_CLIP_PLANE2,cp2a);
	s2->drawImage();
	glTranslated(0,-0.05,0);
...
{
	/// Open window
	if( !openWindow3D( "CSG difference" ) ) return 1;
	changeGround();
	changeGround();
	changeBrightness();
	...
	while( isRunning() )
	{
		...
		s1->setRadius(r1);
		s2->setRadius(r2);
		s1a->setScale({r1*1.01,r1*1.01,r1*1.02});
		s2a->setScale({r2*1.01,r2*1.01,r2*1.02});
