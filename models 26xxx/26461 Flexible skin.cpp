...
/// Project:	FMI 3D (2013)
/// File:		26461 Flexible skin
/// Topic:		26 Surfaces
/// Slide:		46
///	About:		Flexible skin attached on skeleton
///
/// History:
///		2013.06.22/PB	Initial implementation
///     2013.09.14/PB   Changed button image, simlified objects
...
void drawScene()
{
	...
	glLineWidth(2.2-sqrt(level));

	if( level<2 )
        glEnable(GL_POINT_SMOOTH);
    else
        glDisable(GL_POINT_SMOOTH);
	glPointSize(9-2*level);
...
void showWireframe()
{
	...
	for( int i=0; i<N; i++ )
	{
		switch( mode )
		{
			case 0: subd[i]->setPolygonMode( GL_POINT ); break;
			case 1: subd[i]->setPolygonMode( GL_LINE ); break;
			case 2:	subd[i]->setPolygonMode( GL_FILL );
		}
		subd[i]->setLighting(mode==2);
		subd[i]->setShininess(mode==2?10:0);
	}
}
...
{
	/// Open window
	if( !openWindow3D( "Flexible skin" ) ) return 1;
	noGround();
	...
	butNL = new Button2D( "buttons/button_n.png",GLFW_KEY_SPACE,nextLevel,level,5 );
	...
	{
		subd[i] = new Subdiv3D();
		subd[i]->setColor( {0,0,1} );
		subd[i]->setLevel( level );
		subd[i]->setPolygonMode( GL_FILL );
		subd[i]->setShininess(10);
	}
	skeleton();
	showWireframe();
