...
/// Project:	FMI 3D (2013)
/// File:		26181 Pagoda animated by translation.cpp
/// Topic:		26 Deformations
/// Slide:		18
///	About:		A pagoda animated by translation of floors
///
/// History:
...
void Pagoda3D::drawImage( )
{
	...
	double d = -1+8*fabs(sin(t*spd+ofs));
...
{
	/// Open window
	if( !openWindow3D( "Pagodas animated by translation" ) ) return 1;
	changeGround();
	changeGround();
	changeBrightness();
	...
	for(int i=0; i<N; i++)
	{
		pagoda[i] = new Pagoda3D( irandom(5,20) );
