...
/// Project:	FMI 3D (2013)
/// File:		13501 Grass double wave.cpp
/// Topic:		13 Physics
/// Slide:		50
///	About:		Grass double wave
///
/// History:
...
void drawScene()
{
	...
	for( int i=0; i<n; i++ )
	{
		...
		{
			v1.vertex();
			vect_t v2 = {wind*sin(xofs[i].x*(1+j/3.0)),wind*sin(yofs[i].x*(1+j/3.0)),5};
...
{
	/// Open window
	if( !openWindow3D( "Grass double wave" ) ) return 1;
