...
/// Project:	FMI 3D (2013)
/// File:		20361 Orthogonal projection.cpp
/// Topic:		20 Projections
/// Slide:		36
///	About:		Orthogonal projection without matrix
///
/// History:
...
{
	/// Open window
	if( !openWindow3D( "Orthogonal projection" ) ) return 1;
	noGround();
	...
	while( isRunning() )
	{
		...
		for(int i=0; i<N; i++)
		{
			...
			Q[i]->setCenter( {P[i]->getCenter().x,P[i]->getCenter().y,f} );
		}
	}
	return 0;
}
