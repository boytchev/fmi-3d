...
/// Project:	FMI 3D (2013)
/// File:		11221 Incorrect synchronization
/// Topic:		11 Animation
/// Slide:		22
///	About:		Incorrect synchronization of motions
///
/// History:
...
int main()
{
	if( !openWindow3D( "Incorrect synchronization" ) ) return 1;
	changeGround();
	changeGround();
	changeGround();
	...
	while( isRunning() )
	{
		int f = int((t-t0)*80)%(n+3*w)-w;

		if( f<0 ) cube[1]->setCenter({0,0,13}); else
		if( f<n ) cube[1]->setCenter({0,0+8.0/n*f,13-2*8.0/n*f});
	}
	return 0;
}
