...
/// Project:	FMI 3D (2013)
/// File:		08201 Rasterisation - float division.cpp
/// Topic:		08 Rasterisation
/// Slide:		20
///	About:		Floatint point rasterisation of segment
///
/// History:
...
void drawLine( double px,double py,double qx,double qy )
{
	if( fabs( px-qx )<1 && fabs( py-qy )<1 ) return;
	double mx = ( px+qx )/2;
	double my = ( py+qy )/2;
	a[int(mx+0.5)][int(my+0.5)]->setColor( {0.5,0.5,1} );
	a[int(mx+0.5)][int(my+0.5)]->setScale( {0.6,0.1,0.6} );
...
int main()
{
	...
	/// Open window
	if( !openWindow3D( "Rasterisation: float division" ) ) return 1;
	changeBrightness();
	changeGround();
	changeGround();
	changeGround();
